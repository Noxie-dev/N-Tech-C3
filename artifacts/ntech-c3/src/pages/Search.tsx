  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        ...
        <Input aria-label="Created to" type="date" value={to} onChange={(event) => setTo(event.target.value)} />
      </div>
      <div className="relative">
        ...
        <Input
          ...
          autoFocus
        />
        {isFetching && <Spinner className="absolute right-4 top-1/2 -translate-y-1/2" />}
      </div>

      {error && (
        <p role="alert" className="rounded border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
      ...
      {normalizedQuery.length < 2 ? (
        ...
        <div className="space-y-3">
          {data.map((result) => (
            <Link key={`${result.entityType}-${result.entityId}`} href={result.path} className="block">
