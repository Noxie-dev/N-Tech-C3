...
import { useListStories, useCreateStory, useListWorkspaces } from '@workspace/api-client-react';
...
export function Stories() {
  ...
  const queryWorkspaceId = typeof window === 'undefined' ? undefined : Number(new URLSearchParams(window.location.search).get('workspaceId')) || undefined;
  const workspaceId = Number(routeParams.workspaceId) || queryWorkspaceId;
  ...
  const { data: stories, isLoading } = useListStories({
    ...
    storyType: typeFilter || undefined,
    workspaceId,
  });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{workspaceId ? 'Stories in this Workspace.' : 'Transform engineering intelligence into durable narratives.'}</p>
          </div>
        </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2 col-span-2">
                  <label className="text-xs font-mono text-muted-foreground uppercase">Workspace</label>
                  <Select name="workspaceId" defaultValue={workspaceId || ''} required>
                    <option value="" disabled>Choose a Workspace</option>
                    {workspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name}</option>)}
                  </Select>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-mono text-muted-foreground uppercase">Story Type</label>
                  <Select name="storyType" defaultValue="Other">
                    {['EngineeringJournal', 'BlogArticle', 'SocialSeries', 'CaseStudy', 'TechnicalDocumentation', 'ADR', 'ResearchNote', 'LearningNote', 'ProductUpdate', 'ChangelogNarrative', 'InternalMemo', 'Presentation', 'Other'].map((type) => <option key={type}>{type}</option>)}
                  </Select>
                </div>
                <div className="space-y-2">
        ...
        <CardHeader className="py-4 border-b border-border/50 bg-muted/20">
          <div className="flex flex-col sm:flex-row justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              ...
              <Select className="w-full sm:w-[180px] font-mono text-xs" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}>
                <option value="">ALL TYPES</option>
                {['EngineeringJournal', 'BlogArticle', 'CaseStudy', 'TechnicalDocumentation', 'ADR', 'ResearchNote', 'Presentation', 'Other'].map((type) => <option key={type}>{type}</option>)}
              </Select>
            </div>
          </div>
        </CardHeader>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/50">
                {isLoading ? (
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-muted-foreground font-mono text-xs">Querying database...</td></tr>
                ) : stories?.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-10 text-center text-muted-foreground"><p className="font-semibold text-foreground">Every great engineering insight begins as a Story.</p><p className="mt-1 text-xs">Capture an idea, attach evidence, and build something worth sharing.</p></td></tr>
                ) : (
                  stories?.map((story) => (
                    <tr key={story.id} className="hover:bg-muted/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-xs font-mono">{story.storyType || 'Other'}</td>
                      <td className="px-6 py-4">
