...
function openCapture(type: string) {
  window.dispatchEvent(
    new CustomEvent("ntc3:quick-capture", { detail: { type } }),
  );
}
      ...
      <header className="flex h-14 shrink-0 items-center border-b border-[#272b34] bg-[#090b10] px-5">
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            ...
            className="rounded-md p-2 text-[#aeb6c2] hover:bg-[#161c24] hover:text-white"
          >
          ...
          <button className="flex items-center gap-2 rounded-md px-1.5 py-1 hover:bg-[#161c24]">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#2f80ff] text-xs font-semibold text-white">
              N
            </span>
            <span className="hidden text-xs text-[#f5f7fa] sm:inline">
              NaniTech
            </span>
        ...
        <aside className="hidden w-[204px] shrink-0 flex-col border-r border-[#272b34] bg-[#090b10] px-3 py-5 md:flex">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return item.planned ? (
                <span
                  ...
                  aria-disabled="true"
                  className="flex cursor-not-allowed items-center gap-3 rounded-md px-3 py-2 text-xs text-[#687281] opacity-70"
                >
                  {content}
                </span>
              ) : (
          ...
          <section className="mt-auto rounded-[10px] border border-[#30343d] bg-[#0d1118] p-3">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="font-mono text-[10px] font-semibold tracking-[0.08em] text-[#cbd2dc]">
                QUICK CAPTURE
              </h2>
