...
import { createVaultBackup, extractVaultBackup, validateArchiveListing } from './vault-backup.mjs';
...
describe('Vault backup conformance', () => {
  it('round-trips database and Evidence lifecycle/recovery fixtures', async () => {
    ...
    expect(await readFile(path.join(restored, 'database', 'ntc3.sqlite'), 'utf8'))
      .toContain('active archived failed ingest');
  });

  it('rejects traversal and absolute archive entries', () => {
    expect(() => validateArchiveListing('./database/ntc3.sqlite\n../escape')).toThrow('Unsafe');
