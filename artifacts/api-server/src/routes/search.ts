...
router.get('/search', (req, res) => {
  ...
  if (query.data.projectId != null) {
    ...
    params.push(query.data.projectId, query.data.projectId, query.data.projectId, query.data.projectId);
  }
  ...
  if (query.data.to) {
    conditions.push(`datetime(coalesce(${createdAtExpression}, '1970-01-01')) < datetime(?, '+1 day')`);
  ...
  res.json(GlobalSearchResponse.parse(rows.map((row) => {
